...
bool DFRobot_BME680::begin(void)
{
  uint8_t id = this->readID();
  if(id == 0x61) {
    this->writeReg(BME680_STATUS, 0x10);
...
float DFRobot_BME680::readGas(void)
{
  uint16_t gas = 0;
	...
	this->readReg(BME680_FIELD_0_GAS_RL_MSB, 2, rawData);
...
{
  uint16_t gas = this->readGas();
  uint8_t var = 0;
	this->readReg(BME680_FIELD_0_GAS_RL_LSB, 1, &var);
